  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let targetZ = -1, targetY = -1, targetX = -1;
        currentGridData.forEach((layer, z) => layer.forEach((row, y) => row.forEach((cell, x) => {
            if (cell['upload-point']) { targetZ = z; targetY = y; targetX = x; }
        })));
